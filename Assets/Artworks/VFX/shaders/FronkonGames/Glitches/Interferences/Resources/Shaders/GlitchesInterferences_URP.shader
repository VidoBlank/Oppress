    ...
    Pass
    {
      ...
      half4 GlitchesFrag(const VertexOutput input) : SV_Target
      {
        ...
        if (mod(screenCoord.y, rnd) > rnd / 8.0)
        {
          ...
          uv = float2(uv.x + of * _MainTex_TexelSize.x, uv.y + of * 0.5 * _MainTex_TexelSize.y);
          pixel.rgb = SAMPLE_MAIN(uv).rgb;

          float2 offset = of * rnd * float2(5.0, 0.0) * _Offset * _MainTex_TexelSize.xy;
        ...
        {
          uv = float2(uv.x - rnd * 4.0 * _MainTex_TexelSize.x, uv.y + rnd * 0.5 * _MainTex_TexelSize.y);
          pixel.rgb = SAMPLE_MAIN(uv).rgb;

          float2 offset = rnd * float2(50.0, 0.0) * _Offset * _MainTex_TexelSize.xy;
          interfarence.r = SAMPLE_MAIN(uv + offset).r;
          interfarence.g = pixel.g;
          interfarence.b = SAMPLE_MAIN(uv - offset).b;
        }
